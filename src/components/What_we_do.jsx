
export const What_we_do = () => {
  return (
    <div className="container py-12">
        <div className="title">
            What you 
            <span className="pl-2 text-[#FFCA1D]">
                can do ?
            </span>
        </div>
        <div className="flex flex-wrap gap-14 lg:gap-0 lg:justify-between justify-center">
            
            <div className="w-[300px] flex flex-col items-center rounded-xl p-5 shadow-lg">
                <div className="w-10 bg-[#FED369] h-10 rounded-2xl flex justify-center items-center">
                    <img src="/icons/graph.png" className="w-6 h-6" alt="g" />
                </div>
                <h2 className="text-[#2B3377] text-[22px] py-2 text-center"> Market Analysis</h2>
                <p>
                    A market analysis is a quantitative and 
                    qualitative assessment of a market.
                    It looks into the size of the market.
                </p>
            </div>
            <div className="w-[300px] flex flex-col items-center rounded-xl p-5 shadow-lg">
                <div className="w-10 bg-[#FED369] h-10 rounded-2xl flex justify-center items-center">
                    <img src="/icons/graph.png" className="w-6 h-6" alt="g" />
                </div>
                <h2 className="text-[#2B3377] text-[22px] py-2 text-center"> Market Analysis</h2>
                <p>
                    A market analysis is a quantitative and 
                    qualitative assessment of a market.
                    It looks into the size of the market.
                </p>
            </div>
            <div className="w-[300px] flex flex-col items-center rounded-xl p-5 shadow-lg">
                <div className="w-10 bg-[#FED369] h-10 rounded-2xl flex justify-center items-center">
                    <img src="/icons/graph.png" className="w-6 h-6" alt="g" />
                </div>
                <h2 className="text-[#2B3377] text-[22px] py-2 text-center"> Market Analysis</h2>
                <p>
                    A market analysis is a quantitative and 
                    qualitative assessment of a market.
                    It looks into the size of the market.
                </p>
            </div>
        </div>
    </div>
  )
}
